...
uses
  ...
  BufDataset, Bass, LCLIntf, LCLType, LResources; {LAZARUS: JPEG removido — LCL registra JPEG auto}
...
begin
  ...
  {LAZARUS: TBufDataset.EmptyDataSet n/a — deletar registros}
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          DM.cdsSLIDE_MUSICA.FieldByName('COR_LETRA').Value := ColorToString(fmIndex.corTituloMusica.ButtonColor) {LAZARUS: TbsSkinColorButton.ColorValue→TColorButton.ButtonColor};
        end
        else
        begin
          ...
          begin
            if uCor = '' then
            begin
              DM.cdsSLIDE_MUSICA.FieldByName('COR_LETRA').Value := ColorToString(fmIndex.corTextoRepetido.ButtonColor) {LAZARUS: TbsSkinColorButton.ColorValue→TColorButton.ButtonColor};
              uCor := 'S';
            end
            else
            begin
              DM.cdsSLIDE_MUSICA.FieldByName('COR_LETRA').Value := ColorToString(fmIndex.corTextoMusica.ButtonColor) {LAZARUS: TbsSkinColorButton.ColorValue→TColorButton.ButtonColor};
              uCor := '';
            end;
          end
          else
        ...
        DM.cdsSLIDE_MUSICA.FieldByName('COR_LETRA_AUX').Value := ColorToString(fmIndex.corTextoRepetido.ButtonColor) {LAZARUS: TbsSkinColorButton.ColorValue→TColorButton.ButtonColor};
      end
      else
      begin
      ...
      begin
        ...
        begin
          DM.cdsSLIDE_MUSICA.FieldByName('COR_FUNDO').Value := ColorToString(fmIndex.corFundoMusica.ButtonColor) {LAZARUS: TbsSkinColorButton.ColorValue→TColorButton.ButtonColor};
...
begin
  ...
  begin
    ...
    {DM.cdsSLIDE_MUSICA.LogChanges := False;} {LAZARUS: TBufDataset não tem LogChanges}
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        imgFundo.Visible := False;
        fMusica.Color := 1;
        {fMusica.TransparentColor := True;} {LAZARUS: TForm.TransparentColor não existe no LCL}
        {fMusica.TransparentColorValue := fMusica.Color;} {LAZARUS: não existe no LCL}
      end
      else
      begin
        fMusica.Color := clBlack;
        imgFundo.Visible := true;
        {fMusica.TransparentColor := False;} {LAZARUS: não existe no LCL}
      end;
    end
    else
    begin
...
//  else imgFundo.Visible := False;
  ...
  begin
    ...
    if (fmIndex.lerParam('Musicas', 'ModoOperador', '1') = '1')
      then fMusicaOperador.gSlide.Max {LAZARUS: MaxValue→Max (TProgressBar)} := next_time;
  end
  else
  begin
    next_time := -1;
    Edit2.Text := '-1';
    if (fmIndex.lerParam('Musicas', 'ModoOperador', '1') = '1')
      then fMusicaOperador.gSlide.Max {LAZARUS: MaxValue→Max (TProgressBar)} := fMusicaOperador.gSlide.Min {LAZARUS: MinValue→Min (TProgressBar)}+1;
  end;

  if (fmIndex.lerParam('Musicas', 'ModoOperador', '1') = '1') then
  begin
    fMusicaOperador.gSlide.Position {LAZARUS: Value→Position (TProgressBar)} := fMusicaOperador.gSlide.Min {LAZARUS: MinValue→Min (TProgressBar)};
    fMusicaOperador.gSlideTotal.Position {LAZARUS: Value→Position (TProgressBar)} := fMusicaOperador.gSlide.Min {LAZARUS: MinValue→Min (TProgressBar)};
...
begin
  ...
  begin
    ...
    begin
      if next_time < 0
        then fMusicaOperador.gSlide.Max {LAZARUS: MaxValue→Max (TProgressBar)} := len
        else fMusicaOperador.gSlide.Max {LAZARUS: MaxValue→Max (TProgressBar)} := next_time;
      fMusicaOperador.gSlide.Position {LAZARUS: Value→Position (TProgressBar)} := pos;
      fMusicaOperador.gSlideTotal.Max {LAZARUS: MaxValue→Max (TProgressBar)} := len;
      fMusicaOperador.gSlideTotal.Position {LAZARUS: Value→Position (TProgressBar)} := pos;
    ...
    begin
      if next_time < 0
        then fMusicaRetorno.gSlide.Max {LAZARUS: MaxValue→Max (TProgressBar)} := len
        else fMusicaRetorno.gSlide.Max {LAZARUS: MaxValue→Max (TProgressBar)} := next_time;
      fMusicaRetorno.gSlide.Position {LAZARUS: Value→Position (TProgressBar)} := pos;
      fMusicaRetorno.gSlideTotal.Max {LAZARUS: MaxValue→Max (TProgressBar)} := len;
      fMusicaRetorno.gSlideTotal.Position {LAZARUS: Value→Position (TProgressBar)} := pos;
    ...
    begin
      fMusicaOperador.gSlide.Min {LAZARUS: MinValue→Min (TProgressBar)} := next_time;
      tmrTempo.Enabled := False;
      acaoSlide('prox',False);
    end;
  end
